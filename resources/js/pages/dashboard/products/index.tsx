...
export default function ProductsIndex({ products, filters }: Props) {
    ...
    const columns: ColumnDef<Product>[] = [
            ...
            cell: ({ row }) => (
                <div>
                    {row.original.discount_percentage ? (
                        ...
                        <div className="font-medium">{row.original.formatted_price}</div>
                    )}
                </div>
            ),
        },
            ...
            cell: ({ row }) => {
                ...
                return (
                    <div className="flex items-center gap-2">
                        <Badge
                            variant={isOut ? 'destructive' : isLow ? 'warning' : 'secondary'}
                        >
                            {stock}
                        </Badge>
                        {isLow && !isOut && (
        ...
        },
        {
            ...
            header: 'Status',
            cell: ({ row }) => (
                <Badge variant={statusColors[row.original.status] as 'default' | 'secondary' | 'outline'}>
        ...
        },
        {
            ...
            cell: ({ row }) => (
                ...
                >
                    ...
                    {row.original.moderation_status === 'approved' && 'Approved'}
                    {row.original.moderation_status === 'rejected' && 'Rejected'}
                </Badge>
            ),
        },
        {
            ...
            cell: ({ row }) => (
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                        ...
                        {row.original.seller && (
                            <DropdownMenuItem asChild>
                                <Link href={`/dashboard/users/${row.original.seller.id}`}>
                                    <User />
                                    View Seller
                                </Link>
                            </DropdownMenuItem>
            ...
            <AlertDialog open={openDeleteDialog} onOpenChange={setOpenDeleteDialog}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Are you sure you want to delete "{selectedProduct?.name_bn}"?
